...
// echo 'esta con rif fijo para pruebas';
	...
	if($action == 'ajax'){
		...
		$estatus_pendiente=1;
		$consulta="select count(idregistro) as numrows from timbresfiscales where statustimbre=$estatus_pendiente";
		// echo $consulta;
		...
		$paginas="select *, DATE_FORMAT(now(),'%m/%d/%Y') as hoy, DATE_FORMAT(fechatimbre,'%m/%d/%Y') as fechas, DATE_FORMAT(fechapago,'%m/%d/%Y') as fechap from timbresfiscales where statustimbre='$estatus_pendiente' order by fechatimbre LIMIT $offset,$per_page ";
		...
		<table class="table table-bordered">
			...
			while($row = $con->fetch_assoc($query)){
				...
				<tr>
					...
					echo substr($fila2['concepto'],0,40);?></td>
					<td><?php echo $row['montobs'];?></td>
					<td><?php echo $row['montout'];?></td>
					<td><?php echo $row['nroplanilla'];?></td>
					<td><?php echo $row['anombrede'];?></td>
