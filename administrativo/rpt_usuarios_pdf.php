...
$con->conectar();
$sql_360="select * from ingreso_usuario order by nombre ";// limit 30"; //  limit 20";
...
$alto=4;
$salto=$alto;
$w=array(18,90,35,15,15,25); //,25,25,25,25,25,25);
$p[0]=10;
...
$pdf->Open();
$linea=encabeza_l_usuarios($header,$w,$p,$pdf,$salto,$alto, $con);
...
while ($r_nopr = $con->fetch_assoc($a_360)){
	...
	$linea+=$alto;
	$pdf->SetX($p[0]);
	$pdf->Cell($w[0],$alto,substr(trim($r_nopr["codigousuario"]),-8),0,0,'LRTB',0);
	$pdf->SetX($p[1]);
	$pdf->Cell($w[1],$alto,$r_nopr["nombre"],0,0,'LRTB',0); 
	$pdf->SetX($p[2]);
	if (strlen(trim($r_nopr["email"])) < 20)
		$pdf->Cell($w[2],$alto,$r_nopr["email"],0,0,'LRTB',0);  
	else $pdf->Cell($w[2],$alto,substr(trim($r_nopr["email"]),0,8).'...'.substr(trim($r_nopr["email"]),0,-8),0,0,'LRTB',0);  
	$pdf->SetX($p[3]);
...
////////////////////////////////////////////////////
function encabeza_l_usuarios($header,$w,$p,&$pdf,$salto,$alto, $con)
{
$pdf->AddPage();
$linea=25;
$pdf->SetY($linea);
...
$pdf->MultiCell(0,0,"Reporte de Usuarios Sistema Administrativo al ".$con->convertir_fechadmy(substr($con->ahora(),0,10)),0,'C',0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(240);
