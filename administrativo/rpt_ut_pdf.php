...
$con->conectar();
$sql_360="select * from unidadtributaria order by fechainicio ";// limit 30"; //  limit 20";
...
$alto=4;
$salto=$alto;
$w=array(58,58,58); // 35,15,15,25); //,25,25,25,25,25,25);
$p[0]=10;
...
while ($r_nopr = $con->fetch_assoc($a_360)){
	...
	$linea+=$alto;
	$pdf->SetX($p[0]);
	$pdf->Cell($w[0],$alto,$con->convertir_fechadmy($r_nopr["fechainicio"]),0,0,'LRTB',0);
	$pdf->SetX($p[1]);
	$pdf->Cell($w[1],$alto,$con->convertir_fechadmy($r_nopr["fechafinal"]),0,0,'LRTB',0); 
	$pdf->SetX($p[2]);
	$pdf->Cell($w[2],$alto,number_format($r_nopr["montout"],2,',','.'),0,0,'LRTB',0);  
/*
	$pdf->SetX($p[3]);
...
$pdf->MultiCell(0,0,"Reporte de Unidades Tributarias al ".$con->convertir_fechadmy(substr($con->ahora(),0,10)),0,'C',0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(240);
