...
----------------------------*/
	...
	if($action == 'ajax'){
		...
		$estatus_pendiente=1;
		$consulta="select count(fechainicio) as numrows from unidadtributaria";
		// echo $consulta;
		...
		$paginas="select * FROM unidadtributaria order by fechainicio DESC  LIMIT $offset,$per_page ";
		...
		<table class="table table-bordered">
			...
			while($row = $con->fetch_assoc($query)){
				echo '<tr>';
				echo '<td>'.substr($row['idregistro'],-8).'</td>';
				echo '<td>'.$con->convertir_fechadmy($row['fechainicio']).'</td>';
				echo '<td>'.$con->convertir_fechadmy($row['fechafinal']).'</td>';
				echo '<td>'.number_format($row['montout'],2,'.',',').'</td>';
				echo '<td>'.$con->convertir_fechadmy($row['fechagenerada']).'</td>';
				echo '<td>';
				$ya=$con->ahora();
				if (($row['fechafinal']>$ya) and ($row['fechainicio'] > $ya))
				{
					echo '<button type="button" class="btn btn-info" data-toggle="modal" data-target="#dataUpdate" data-id="'.$row['idregistro'].'" data-monto="'.$row['montout'].'"><i class="glyphicon glyphicon-edit"></i> Modificar</button>';
					echo '<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#dataDelete" data-id="'.$row['idregistro'].'" data-inicio="'. $row['fechainicio'].'" data-final="'. $row['fechafinal'].'" data-monto="'.$row['montout'].'" ><i class="glyphicon glyphicon-trash"></i> Eliminar</button>';
				}
				echo '</td>';
